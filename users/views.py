...
def add_to_cart(request,product_id):
   if(request.user.is_authenticated):
      ...
      if(in_cart.count()>0):
         ...
         cart.save()
         messages.success(request,"product is already to the cart")
         return redirect("/")
...
def cart_item_count(request):
   user_specific_products= Cart.objects.filter(uid=request.user.id)
   return user_specific_products.count()
...
def cart(request):
   ...
   data['cart_items']= cart_item_count(request)
   ...
   return render(request,'users/cart.html',context=data)
...
def update_cart_quantity(request,flag,cart_id):
       ...
       print("actual_quantity",actual_quantity)
       if (flag=='inc'):
...
def order_summary(request):
   ...
   data1['cart_items']= cart_item_count(request)
   #getting cart items based on quantity
   for item in cart_items:
      total_items+=item.quantity
      total_price+=(item.quantity*item.pid.price)
   ...
   return render(request,'users/order_summary.html',context=data1)
